...
class Game extends Component {
  ...
  componentDidMount() {
    ...
    axios.get(serverBase + this.state.gameCode).then(function (response) {
      let message = response.data;
      if (message.success) {
        if ((message.success = "president")) that.connect();
        else {
          that.setState({
            freeze: true,
          });
  ...
  render() {
    return (
      <div className="gameField">
        {this.state.playerName && !this.state.ready && (
          <button className="oldButton" onClick={this.ready.bind(this)}>
            Ready
          </button>
        )}
        ...
        <div className="playerAndField">
          <div className="playerDiv">
            {this.state.playerName &&
              this.state.otherPlayers.map((obj, i) => (
                <Player
                  key={"player" + i}
                  currentTurn={obj.current_turn}
                  name={obj.name}
                  cards={obj.num_cards}
                />
              ))}
        ...
        <Chat
          ...
          ws={this.ws}
          update={(msg, old) => this.newMessage(this, msg, old)}
        />

        {!this.state.playerName && (
          <GetPlayerName
            onNameChange={this.onNameChangeHandler.bind(this)}
            submitName={this.getPlayerName.bind(this)}
          />
        )}
        ...
        <Rodal
          ...
          onClose={this.hideScoreBoard.bind(this)}
        >
          {this.state.results}
        </Rodal>
      </div>
...
function GetPlayerName(props) {
  return (
    <div className="singleForm">
      ...
      />
      <button className="oldButton" onClick={props.submitName}>
        Enter
      </button>
    </div>
  );
